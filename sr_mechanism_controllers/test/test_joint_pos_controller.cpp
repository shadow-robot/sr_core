...
using controller::SrhJointPositionController;
...
public:
  ...
  explicit TestJointPositionController(boost::shared_ptr <control_toolbox::Pid> pid) :
          TestControllers()
  {
    this->pid = pid;
  ...
  void init_controller()
  {
    ...
    controller::SrhJointPositionController *sr_control_tmp =
            dynamic_cast< controller::SrhJointPositionController * >(control_tmp);
...
// TESTING A PURE P CONTROLLER
  ...
  for (unsigned int i = 0; i < nb_values; ++i)
  {
    ...
    EXPECT_TRUE(fabs(ctrl_output - expected_values_one_sec[i])
                < 0.001);
  }
  for (unsigned int i = 0; i < nb_values; ++i)
  {
