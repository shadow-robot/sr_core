...
// mature yet.
  ...
  static void getQuinticSplineCoefficients(double start_pos, double start_vel, double start_acc,
                                           double end_pos, double end_vel, double end_acc, double time,
 ...
 */
          ...
          nh_tilde("~"), use_sendupdate(false)
  {
    ...
    if (ros::service::waitForService("controller_manager/list_controllers", 20000))
    {
      ...
      ros::ServiceClient controller_list_client = nh.serviceClient<controller_manager_msgs::ListControllers>(
  ...
  void JointTrajectoryActionController::execute_trajectory(const control_msgs::FollowJointTrajectoryGoalConstPtr &goal)
  {
    ...
    for (size_t i = 0; i < goal->trajectory.points.size(); ++i)
    {
      ...
      {
        seg.start_time =
                (goal->trajectory.header.stamp + goal->trajectory.points[i].time_from_start).toSec() - durations[i];
      }
      ...
      for (size_t j = 0; j < goal->trajectory.joint_names.size(); ++j)
      {
        ...
        if (!velocities.empty())
        {
          velocities[joint_state_idx_map[goal->trajectory.joint_names[j]]] = goal->trajectory.points[i].velocities[j];
        }
        if (!positions.empty())
        {
          positions[joint_state_idx_map[goal->trajectory.joint_names[j]]] = goal->trajectory.points[i].positions[j];
        }
      }
...
//  trajectory_msgs::JointTrajectoryPoint trajectory_step;
    ...
    while (ros::ok())
    {
      ...
      {
        ROS_DEBUG("trajectory is finished %f<%f", (traj[traj.size() - 1].start_time + traj[traj.size() - 1].duration),
                  time.toSec());
        break;
      }
      ...
      if (seg == -1)
      {
        if (traj.size() == 0)
          ...
          ROS_ERROR("No earlier segments.  First segment starts at %.3lf (now = %.3lf)", traj[0].start_time,
                    time.toSec());
        success = false;
        break;
      }
  ...
  void JointTrajectoryActionController::commandCB(const trajectory_msgs::JointTrajectoryConstPtr &msg)
  {
    ...
    for (size_t i = 0; i < msg->points.size(); ++i)
    {
      ...
      if (msg->header.stamp == ros::Time(0.0))
      {
        ...
        ROS_DEBUG("Segment %zu start time A %f,time_from_start %f, duration, %f", i, seg.start_time,
                  msg->points[i].time_from_start.toSec(), durations[i]);
      }
      else
      {
      ...
      for (size_t j = 0; j < joint_names_.size(); ++j)
      {
        if (!accelerations.empty())
        {
          accelerations[j] = msg->points[i].accelerations[j];
        }
        if (!velocities.empty())
        {
          velocities[j] = msg->points[i].velocities[j];
        }
        if (!positions.empty())
        {
          positions[j] = msg->points[i].positions[j];
        }
      }
