...
namespace shadow_robot
{
  ...
  void MotorTest::run_test(diagnostic_updater::DiagnosticStatusWrapper &status)
  {
    ...
    {
      status.summary(diagnostic_msgs::DiagnosticStatus::ERROR,
                     "Failed to switch from controller " + current_ctrl + " to controller " + effort_ctrl +
                     " - aborting test.");
      return;
    }
    ...
    {
      status.summary(diagnostic_msgs::DiagnosticStatus::ERROR,
                     "Failed to switch from controller " + effort_ctrl + " to controller " + current_ctrl +
                     " - aborting test.");
      return;
    }
  ...
  void MotorTest::diagnostics_agg_cb_(const diagnostic_msgs::DiagnosticArray::ConstPtr &msg)
  {
    ...
    {
      if (msg->status[status_i].name.find("SRDMotor " + boost::algorithm::to_upper_copy(joint_name_)) !=
          std::string::npos)
      {
